...
//                if(list.get(i)%2==1 && list.get(j)%2==1) {
...
//                }
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for(int i =0;i<(upperBound-lowerBound)+1;i++) {
            ...
            for(int j = 0;j<a.length;j++) {
                sum+=pow(Character.getNumericValue(a[j]),j+1);
            }
            if(sum==ans) {
                list.add(ans);
            }
    ...
    public static List<String> wildWest(List<String> directions) {
        ...
        //    System.out.println(String.join(", ", directions));
            ...
            for(int j =i+1;j<directions.size();j++) {
                if(ans.equals(directions.get(j))) {
                    directions.remove(j);
                    directions.remove(i);
                    i--;
                    break;
    ...
    public static int queueTime(List<Integer> q, int n) {
        int sum=0;
